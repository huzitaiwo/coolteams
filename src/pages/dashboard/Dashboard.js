...
export default function Dashboard() {
  ...
  const myTasks = documents ? documents.filter(document => {
    let assignedToMe = false
    document.tasks.filter(task => {
  ...
  //   let assignedToMe = false
  //   const myTasks = doc ? doc.tasks.filter(task => {
  //     task.assignedUsersList.forEach(u => {
  //       if (user.uid === u.id) {
  //         assignedToMe = true
  //       }
        ...
        <>
          ...
          <div className="project__grid">
            <div className="my__projects">
              ...
              {myProjects.length === 0 && <h3 className={`warning__text ${mode}`}>Projects you created will show here</h3>}
